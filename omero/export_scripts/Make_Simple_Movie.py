...
format_mimetypes = {MPEG: "video/mpeg", QT: "video/quicktime", WMV: "video/x-ms-wmv"}
...
def download_plane(gateway, pixels, pixels_id, x, y, z, c, t):
    ...
    convert_type = (
        ">"
        + str(x * y)
        + pixelstypetopython.toPython(pixels.getPixelsType().getValue())
    )
...
def calculate_acquisition_time(conn, pixels_id, c_list, tz_list):
    ...
    query = (
        "from PlaneInfo as Info where Info.theZ in (%s) and Info.theT"
...
def add_scalebar(scalebar, image, pixels, command_args):
    ...
    if (
        ...
        or scale_bar_x2 <= 0
        or scale_bar_y <= 0
        or scale_bar_x2 > image_w
    ):
        return image
    draw.line(
        [(scale_bar_x, scale_bar_y), (scale_bar_x2, scale_bar_y)],
        fill=command_args["Overlay_Colour"],
    )
    draw.text(
        ((scale_bar_x + scale_bar_x2) / 2, scale_bar_text_y),
        str(scalebar),
        fill=command_args["Overlay_Colour"],
    )
    return image
...
def calculate_ranges(size_z, size_t, command_args):
    ...
    if "Plane_Map" not in command_args:
        ...
        z_end = size_z - 1
        if (
            "Z_Start" in command_args
            and command_args["Z_Start"] >= 0
            and command_args["Z_Start"] < size_z
        ):
            z_start = command_args["Z_Start"]
        if (
            "Z_End" in command_args
            and command_args["Z_End"] >= 0
            and command_args["Z_End"] < size_z
            and command_args["Z_End"] >= z_start
        ):
            z_end = command_args["Z_End"] + 1
        t_start = 0
        t_end = size_t - 1
        if (
            "T_Start" in command_args
            and command_args["T_Start"] >= 0
            and command_args["T_Start"] < size_t
        ):
            t_start = command_args["T_Start"]
        if (
            "T_End" in command_args
            and command_args["T_End"] >= 0
            and command_args["T_End"] < size_t
            and command_args["T_End"] >= t_start
        ):
...
def write_intro_end_slides(conn, command_args, orig_file_id, duration, size_x, size_y):
    """
...
def write_movie(command_args, conn, omero_image):
    """
    Makes the movie.

    :param conn:          The OMERO connection object
    :param omero_image:   An OMERO image object as returned by script_utils.get_objects
    ...
    c_windows = None
    c_colours = None
    if "ChannelsExtended" in command_args and valid_channels(
        command_args["ChannelsExtended"], size_c
    ):
        c_range = []
        c_windows = []
        c_colours = []
    ...
    frame_no = 1
    omero_image.setActiveChannels([x + 1 for x in c_range], c_windows, c_colours)
    ...
    for tz in tz_list:
        ...
        if "Scalebar" in command_args and command_args["Scalebar"]:
            image = add_scalebar(command_args["Scalebar"], image, pixels, command_args)
        ...
        if "Show_Plane_Info" in command_args and command_args["Show_Plane_Info"]:
    ...
    file_annotation, ann_message = script_utils.create_link_file_annotation(
        ...
        namespace=namespace,
        mimetype=mimetype,
        orig_file_path_and_name=movie_name,
    )
    message += ann_message
...
def run_script():
    ...
    client = scripts.client(
        ...
        scripts.List(
            "Channels", description="The selected channels", grouping="5.1"
        ).ofType(rint(0)),
        scripts.List(
            "ChannelsExtended",
        ...
        scripts.Int(
            "Min_Width", description="Minimum width for output movie.", default=-1
        ),
        scripts.Int(
            "Min_Height", description="Minimum height for output movie.", default=-1
        ),
        scripts.Map(
            "Plane_Map",
            description="Specify the individual planes (instead of using"
            " T_Start, T_End, Z_Start and Z_End)",
            grouping="12",
        ),
        scripts.Object(
            "Watermark",
            description="Specify a watermark as an Original File (png or" " jpeg)",
            default=omero.model.OriginalFileI(),
        ),
        scripts.Object(
            "Intro_Slide",
            description="Specify an Intro slide as an Original File (png or" " jpeg)",
            default=omero.model.OriginalFileI(),
        ),
        scripts.Int(
            "Intro_Duration",
        ...
        scripts.Int(
            "Ending_Duration",
            default=3,
            description="Duration of finishing slide in seconds. Default is 3" " secs.",
        ),
        scripts.Bool(
            "Do_Link",
    ...
    try:
        ...
        omero_images, log_message = script_utils.get_objects(conn, command_args)
